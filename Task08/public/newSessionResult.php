...
<form class = "form_ses" method="get" action = "" id="form" onchange="changeForm(this)">
    <h1 class = "newStText" >Новый результат сессии</h1>
            <fieldset class="fieldset_class" id="fieldset_class">
                <legend> Информация о сессии </legend>
                <h4>Группа:
                    ...
                    <option class = "option_st">Выбрать</option>
                            ...
                            if(isset($_REQUEST['group'])){
                                ...
                                $st = $db->prepare("SELECT s.surname as sur, s.name as nam, s.id as stud_id from students as s join groups as g on g.student_id = s.id where g.direction = '$select_group';");
                                $st->execute();
                                $res_st = $st->fetchAll(PDO::FETCH_ASSOC);
                                    foreach($res_st as $temp){
                                            $value1 = $temp['sur']." ".$temp['nam'];
...
<?php
    if(isset($_REQUEST['group']) && isset($_REQUEST['surname']) && isset($_REQUEST['course']) && isset($_REQUEST['semester']) && isset($_REQUEST['subject']) && isset($_REQUEST['mark'])){
        $inds = $db->prepare("SELECT MAX(id) from session_results;");
        $inds->execute();
        $ress = $inds->fetchColumn();
        $ind1s = $ress+1;
        $ins2=$db->prepare("INSERT INTO session_results (id,student_id, subject_id, points) VALUES (:id,:student_id, :subject_id, :points);");
        $ins2->execute([
        ':id'=> $ind1s,
        ':student_id' => $stud_id,
        ':subject_id' => $sub_id,
